...
def create_model_capm_ff(
    ...
    for i in range(len(data_asset["<CLOSE>"]) - 1):
        returns_asset.append(
            calculate_log_return(data_asset["<CLOSE>"][i], data_asset["<CLOSE>"][i + 1])
        )

    for i in range(len(data_market["<CLOSE>"]) - 1):
        returns_market.append(
            calculate_log_return(
                data_market["<CLOSE>"][i], data_market["<CLOSE>"][i + 1]
            )
        )
    ...
    for i in range(len(data_stock_growth["<CLOSE>"]) - 1):
        returns_stock_growth.append(
            calculate_return(
                data_stock_growth["<CLOSE>"][i], data_stock_growth["<CLOSE>"][i + 1]
            )
        )

    for i in range(len(data_stock_value["<CLOSE>"]) - 1):
        returns_stock_value.append(
            calculate_return(
                data_stock_value["<CLOSE>"][i], data_stock_value["<CLOSE>"][i + 1]
            )
        )
    ...
    for i in range(len(data_big_cap["<CLOSE>"]) - 1):
        returns_stock_big.append(
            calculate_return(data_big_cap["<CLOSE>"][i], data_big_cap["<CLOSE>"][i + 1])
        )

    for i in range(len(data_small_cap["<CLOSE>"]) - 1):
        returns_stock_small.append(
            calculate_return(
                data_small_cap["<CLOSE>"][i], data_small_cap["<CLOSE>"][i + 1]
            )
        )
    ...
    data_begin = pd.concat(
        [returns_asset, returns_market, returns_hml, returns_smb], axis=1
    ).dropna()
