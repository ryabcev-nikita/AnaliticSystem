    ...
    try:
        with Client(token) as client:
            ...
            filtered_instruments = [
                instrument for instrument in instruments_response.instruments
...
def get_fundamentals_assets_to_excel(token, excel_filename=file_path):
    ...
    try:
        ...
        asset_uids = [inst['asset_uid'] for inst in instruments_list if inst.get('asset_uid')]
        ...
        for i in range(0, len(asset_uids), chunk_size):
            ...
            print(f"🔄 Пачка {i//chunk_size + 1}/{(len(asset_uids)-1)//chunk_size + 1} ({len(chunk)} активов)")
            try:
                with Client(token) as client:
                    ...
                    for fundamental in response.fundamentals:
                        ...
                        matching_instrument = next(
                            (inst for inst in instruments_chunk 
                        ...
                        def safe_get(field, default=None):
                            try:
                                ...
                                if val is None:
                                    return default
                                # Обработка дат
                                if field.endswith('_date') and hasattr(val, 'ToDatetime'):
                                    try:
                        ...
                        row = {
                            ...
                            "Рыночная капитализация": fmt_num(safe_get('market_capitalization')),
                            ...
                            "Дивидендная доходность": safe_get('dividend_yield_daily_ttm'),
                            "Свободный денежный поток": fmt_num(safe_get('free_cash_flow_ttm')),
                            "Бета": safe_get('beta'),
